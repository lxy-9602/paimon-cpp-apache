...
IWYU_LOG=$(mktemp -t paimon-cpp-iwyu.XXXXXX)
trap "rm -f $IWYU_LOG" EXIT

IWYU_MAPPINGS_PATH="$ROOT/build_support/iwyu/mappings"
IWYU_ARGS="\
    --mapping_file=$IWYU_MAPPINGS_PATH/paimon-misc.imp \
    --mapping_file=$IWYU_MAPPINGS_PATH/arrow.imp \
...
affected_files() {
  pushd $ROOT > /dev/null
  local commit=$($ROOT/build_support/get-upstream-commit.sh)
  git diff --name-only $commit | awk '/\.(c|cc|cpp|h)$/'
  popd > /dev/null
}
...
if [[ "${1:-}" == "all" ]]; then
  ...
  ALL_FILES=
  IWYU_FILE_LIST=
  for path in $(find $ROOT/src -type f | awk '/\.(c|cc|cpp|h)$/'); do
    if [[ $path =~ $2 ]]; then
